...
// <td class="transactionDate"><span>${testDateStr}</span></td>

test("addTransaction", () => {
	expect(() => {
		new ExpenseManager("test").addTransaction();
	}).toThrow();
});
...
test("add transaction and get all transactions", () => {
	...
	expect(expenseManager.getTransactions().length).toStrictEqual(2);
});

test("add transaction with 0 amount , expect error", () => {
	expect(() => {
		new ExpenseManager("test").addTransaction(0, "message");
	}).toThrow();
});

test("add transactions and check toHTML response", () => {
	let expenseManager = new ExpenseManager("test");
	expenseManager.addTransaction(100, "message", testDate);
	expenseManager.addTransaction(-20, "message2", testDate);
	expect(expenseManager.toHTML()).toBe(
		`<table class="transactionTable"><tr class="transactionRow"><td class="delete" idValue="1234"></td><td class="transactionAmount possitive">100</td><td class="transactionMessage"><span>message</span></td><td class="transactionDate"><span>${testDateStr}</span></td></tr><tr class="transactionRow"><td class="delete" idValue="1234"></td><td class="transactionAmount negative">20</td><td class="transactionMessage"><span>message2</span></td><td class="transactionDate"><span>${testDateStr}</span></td></tr></table>`
	);
});
...
test("add transactions and check getBallance", () => {
	...
	expenseManager.addTransaction(-20, "message2");
	let transactionList = [
		new Transaction(100, "message", ExpenseType.POSITIVE_EXPENSE),
		new Transaction(20, "message2", ExpenseType.NEGATIVE_EXPENSE),
	];
...
test("add transactions and check toHTML response", () => {
	let expenseManager = new ExpenseManager("test");
	expect(expenseManager.toHTML()).toBe(
		'<table class="transactionTable"></table>'
	);
});
