...
class ExpenseManager{
    ...
    addTransaction(amount , message ){
        ...
        let type = amount>0?ExpenseType.POSITIVE_EXPENSE:ExpenseType.NEGATIVE_EXPENSE
        amount = Math.abs(amount);

        let transaction = new Transaction(amount , message , type ,this.testMode?'1234':uuidv4())
    ...
    toHTML(){
        let innerH = this.transactionList.filter(t=> !t.deleted).map(t=> t.toHTML()).reduce((a,b)=>a+b,"");
    ...
    getBallance(){
        let totalBallance = this.transactionList.filter(t=> !t.deleted).map( a=>{ 
    ...
    getPossitiveBallance(){
        let totalBallance = this.transactionList.filter(t=> !t.deleted)
    ...
    deSerialize(seiralData){
        ...
        for(let {amount, type, message,id, createDate, deleted} of data){
            let transactionType = type==1?ExpenseType.POSITIVE_EXPENSE:ExpenseType.NEGATIVE_EXPENSE;
            let transaction = new Transaction(amount, message, transactionType , id, createDate, deleted)
            this.transactionList.push(transaction);
        }
    }
